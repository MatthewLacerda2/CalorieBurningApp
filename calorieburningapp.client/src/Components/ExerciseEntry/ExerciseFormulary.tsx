...
const ExerciseFormulary: React.FC<ExerciseFormProps> = ({ exerciseEntry }) => {
  ...
  const handleRequest = async () => {
    try {
      const url = exerciseEntry ? `http://localhost:5071/api/v1/entries/${exerciseEntry.id}` : 'http://localhost:5071/api/v1/entries';
  ...
  return (
    <Card title={exerciseEntry ? 'Edit Exercise Entry' : 'Add Exercise Entry'}>
      <div className="exercise-form">
        <label>
          Exercise Type:
          <select name="exercise" value={exerciseData.exercise} onChange={handleInputChange}>
        ...
        <label>
          Date & Time:
          <input name="dateTime" type="datetime-local" value={exerciseData.dateTime.toISOString().substring(0, 16)} onChange={handleInputChange} />
        </label>
        <label>
          Title:
          <input name="title" type="text" value={exerciseData.title} onChange={handleInputChange} />
        </label>
        <label>
          Burned Calories:
          <input name="burnedCalories" type="number" value={exerciseData.burnedCalories} onChange={handleInputChange} />
        </label>
        <button className="send-button" onClick={handleRequest}>Send</button>
      </div>
    </Card>
  );
};
